...
**Lessons (Nested Directory Structure):**

```text
curriculum/lessons/
├── P01-M01-L01/
│   ├── lesson.json           # Lesson schema/data (required)
...
**Summaries (Deprecated):** Lesson summaries are now stored as `completion.md` within each lesson directory

### B.2 Rationale

- **Nested directories:** Groups all lesson artifacts together (3 files per lesson)
- **Scalable:** 850 lessons = 850 folders (not 2,550+ files in one directory)
- **Consistent naming:** Always `lesson.json`, `lesson.md`, `completion.md` within each lesson folder
...
curriculum/lessons/P01-M01-L01/lesson.md
curriculum/lessons/P01-M01-L01/completion.md

# State files (flat structure, unchanged)
learner-state/completed/P01-M01-L01.json

# Legacy structure (deprecated, use migration script)
curriculum/lessons/P01-M01-L01.json
curriculum/lessons/P01-M01-L01_lesson_doc.md
curriculum/lessons/P01-M01-L01_response_and_eval.md
```

### B.5 Migration

To migrate existing lessons from flat structure to nested:

```bash
cd /Users/jeffhutting/dev/dev-curriculum
python tools/migrate_lessons.py
python tools/validate.py
git add curriculum/lessons/
git commit -m "refactor(lessons): migrate to nested directory structure"
```
